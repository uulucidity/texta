...
//   Date: 1/27/2024
///////////////////////////////////////////////////////////////////////
class CEvDtMain: public CEvMain {
public:
    typedef CEvMain CExtends;
    typedef CEvDtMain CDerives;
    ...
    virtual int Run(int argc, const char** argv, const char** env) {
        ...
        if ((chars = m_sourcePath.Chars(length)) && (0 < length)) {
            ...
            DBF("() ...failed on ((chars = m_sourcePath.Chars(%d)) && (0 < length = %d))\n", length, length);
        }
    ...
    //     Date: 1/27/2024
    ///////////////////////////////////////////////////////////////////////
    virtual EvError DateFile(const char* chars, const char* chars2) {
        ...
        if ((EvFileSystemEntry::EV_TYPE_NONE < (type = m_targetEntry.Exists(chars2)))) {
            ...
            case EvFileSystemEntry::EV_TYPE_FILE:
                DBF("() ...case EvFileSystemEntry::EV_TYPE_FILE:\n");
                DBF("() DateFile2File(\"%s\", \"%s\")...\n", chars, chars2);
                error = DateFile2File(chars, chars2);
                break;
            case EvFileSystemEntry::EV_TYPE_DIRECTORY:
                DBF("() ...case EvFileSystemEntry::EV_TYPE_DIRECTORY:\n");
                break;
    ...
    ///////////////////////////////////////////////////////////////////////
    // Function: DateFile2File
    //
    //   Author: $author$
    //     Date: 1/27/2024
    ///////////////////////////////////////////////////////////////////////
    virtual EvError DateFile2File(const char* chars, const char* chars2) {
        EvError error = EV_ERROR_FAILED;
        bool complete = false;
        ...
        DBF("() ((0 < (length = m_filePath.Assign(\"%s\"))))...\n", chars2);
            ...
            if ((fileDir = m_filePath.GetFileDirectory(length)) && (0 < (length))) {
                ...
                if (!(error = MakeDirectory(fileDir))) {
                    ...
                    if (!(error = file.Open(chars, EVFILE_MODE_READ_BINARY))) {
                        ...
                        DBF("() (!(error = file2.Open(\"%s\", EVFILE_MODE_READ_BINARY)))...\n", chars2);
                        if (!(error = file2.Open(chars2, EVFILE_MODE_READ_BINARY))) {
                            DBF("() ...(!(%d = file2.Open(\"%s\", EVFILE_MODE_READ_BINARY)))\n", error, chars2);

                            complete = true;
                            DBF("() file2.Close()...\n");
                            file2.Close();
                        } else {
                            DBF("() ...failed on (!(%d = file2.Open(\"%s\", EVFILE_MODE_READ_BINARY)))\n", error, chars2);
                            ...
                            if (!(error = file2.Open(chars2, EVFILE_MODE_WRITE_BINARY))) {
                                ...
                                DBF("() ...failed on (!(%d = file2.Open(\"%s\", EVFILE_MODE_WRITE_BINARY))\n", error, chars2);
                            }
                        }
                        DBF("() file.Close()...\n");
                        file.Close();
                    } else {
                        DBF("() ...failed on (!(%d = file.Open(\"%s\", EVFILE_MODE_READ_BINARY)))\n", error, chars);
                    }
                } else {
                    DBF("() ...failed on (!(%d = MakeDirectory(\"%s\")))\n", error, fileDir);
                }
            } else {
                DBF("() ...failed on ((fileDir = m_filePath.GetFileDirectory(%d)) && (0 < (length = %d)))\n", length, length);
            }
        } else {
            DBF("() ...failed on ((0 < (%d = m_filePath.Assign(\"%s\"))))\n", length, chars2);
        }
        if (complete) {
